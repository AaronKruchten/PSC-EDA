...
form_DAG <- function(measurements,local_data,weight_matrix_save_location,alpha = .001,number_of_appearances = 10){
  ...
  for(i in 1:length(file_names)){
    ...
    if(usable_frame != c("dummy")){
      ...
      output <- pc(suffStat = suffStat,indepTest = disCItest,alpha =alpha,labels = colnames(usable_frame),verbose = TRUE,fixedEdges = fixed_knowledge_matrix_symmetric)
  ...
  weight_matrix_discrete = weight_matrix >= number_of_appearances
  #adding zero converts TRUE/FALSE to numeric
...
#local_data = "/Users/aaronkruchten/Desktop/PCA flows"
#weight_mat_save_location = "/Users/aaronkruchten/Desktop/Data Science Internship/weight_matrix_discrete_test.csv"
#w <- form_DAG(measurements = measurements,local_data = local_data,weight_matrix_save_location = weight_mat_save_location)
